async function main() {
  ...
  await Contract.connect(account2Signer).addProposal(
    "ma premiere proposition : " + loremIpsum
  );
  console.log("addProposal 2");
  await Contract.connect(account2Signer).addProposal(
    "ma seconde proposition : " + loremIpsum
  );
  console.log("addProposal 3");
  await Contract.connect(account2Signer).addProposal(
    "ma troisieme proposition : " + loremIpsum
  );
  console.log("addProposal 4");
  await Contract.connect(account2Signer).addProposal(
    "ma quatrième proposition : " + loremIpsum
  );
  console.log("addProposal 5");
  await Contract.connect(account2Signer).addProposal(
    "ma cinquième proposition : " + loremIpsum
  );
}
